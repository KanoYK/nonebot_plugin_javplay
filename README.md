...
4. 用户播放虚拟影片时，插件发送等待消息并开始后台缓存流程。
5. 插件把用户配置的资源标识提交给 115 任务能力。
6. 默认模式下，插件从 115 获取可用文件地址并推送到 Aria2，本地缓存到 Jellyfin 媒体目录。
7. 实验性的 `115_mount` 模式下，插件只让 115 保存选中的主视频文件，并等待 Jellyfin 从 115 挂载目录看到真实文件。
8. 插件刷新单个 Jellyfin 媒体项，确认真实文件出现后删除虚拟 `.strm`。
...
`javplay_storage_mode` 控制真实文件如何进入 Jellyfin：

| 模式 | 说明 | 是否需要 Aria2 |
| --- | --- | --- |
| `aria2_cache` | 115 离线完成后，插件把 115 直链推给 Aria2，Aria2 下载到本地 Jellyfin 目录 | 需要 |
| `115_mount` / `clouddrive_mount` | 115 离线完成后，Jellyfin 直接读取你挂载到 NAS 的 115 目录 | 不需要 |

`aria2_cache` 是默认稳定模式。`115_mount` / `clouddrive_mount` 需要你已经把 115 网盘目录真正挂载成 Jellyfin Docker 能访问的本地路径；普通“同步文件夹”不算挂载，因为它仍会占用本地空间。

### 本地缓存模式的三个路径

`aria2_cache` 模式下，JavPlay 同时被 NoneBot、Jellyfin、Aria2 使用。它们看到的路径可能不同，但底层必须是同一个目录。
...
`115_mount` / `clouddrive_mount` 模式下，虚拟影片仍放在 `javplay_db_path`，真实影片放在 115 网盘目录。Jellyfin 媒体库需要同时包含虚拟库目录和 115 挂载目录。

| 使用者 | 配置项 | 示例 | 说明 |
| --- | --- | --- | --- |
| 115 网盘 | `javplay_115_savepath` | `/jellyfin/jav/media` | 插件在这里为每部影片创建单独文件夹 |
| Jellyfin 容器 | `javplay_115_mount_jellyfin_path` | `/jellyfin/115/JAV` | Jellyfin 看到的 115 挂载媒体目录 |

同一部影片最终应当对应：

```text
115 网盘中:   /jellyfin/jav/media/ABC-123/ABC-123.mp4
...
```text
http://YOUR_NONEBOT_HOST:14514/trigger/ABC-123.mp4
```

Jellyfin 播放这个地址时，插件只触发后台下载和 Jellyfin 消息提示，不再返回等待动画视频。插件会结合 Jellyfin 当前活跃播放会话确认用户真的在播放该番号，确认后才会加入缓存队列，避免 Jellyfin 客户端预加载旧视频导致误触发。
...
├── downloader_115.py        # 115 登录、任务提交、文件选择、直链或挂载模式
...
Jellyfin 能访问 `javplay_strm_url` 后，播放虚拟影片时会触发插件下载流程，并通过 Jellyfin 消息提示用户等待。
...
```toml
# 存储模式。默认 aria2_cache；想通过 CloudDrive2 跳过 Aria2 时设为 clouddrive_mount
javplay_storage_mode="aria2_cache"
...
# 115 挂载模式，仅 115_mount 模式必填
javplay_115_mount_jellyfin_path="/PATH_IN_JELLYFIN_CONTAINER/115-JAV"
...
| `javplay_115_require_wanted_selection` | 否 | 默认 `true`。无法选择 BT 内部主视频时拒绝添加任务，避免污染媒体库 |
| `javplay_proxy_http` | 否 | 直接请求公开索引页面时使用的 HTTP 代理 |
| `javplay_flaresolverr_url` | 是 | FlareSolverr 地址，不要带 `/v1`；如果配置了 `javplay_proxy_http` 可留空 |
| `javplay_flaresolverr_proxy` | 否 | 传给 FlareSolverr 的上游代理 |
| `javplay_aria2_rpc` | `aria2_cache` 必填 | Aria2 JSON-RPC 地址 |
| `javplay_aria2_secret` | `aria2_cache` 必填 | Aria2 RPC Secret |
| `javplay_aria2_dir` | `aria2_cache` 必填 | Aria2 容器内缓存目录 |
...
| `javplay_strm_url` | 是 | 写入 `.strm` 的下载触发 URL，推荐包含 `{video_id}` |
...
仅 `javplay_storage_mode="aria2_cache"` 时需要 Aria2。插件通过 Aria2 JSON-RPC 处理 115 返回的可用文件地址。
...
`javplay_storage_mode="115_mount"` 会跳过 Aria2：插件提交 115 离线任务后，只等待 115 网盘中出现选中的主视频文件，然后刷新 Jellyfin。

这种模式要求你已经用 NAS、系统工具或第三方挂载工具把 115 网盘目录挂成 Jellyfin Docker 能读取的本地目录。Jellyfin 媒体库中需要额外添加这个容器内路径：
...
```text
/jellyfin/jav/media/ABC-123/ABC-123.mp4
```
...
### Jellyfin 已请求虚拟影片，但没有开始缓存

插件会确认 Jellyfin 当前活跃播放会话中的番号与触发 URL 中的番号一致。没有匹配会话时不会进入缓存队列。
