...
| `115_mount` | 115 离线完成后，Jellyfin 直接读取你挂载到 NAS 的 115 目录 | 不需要 |

`aria2_cache` 是默认稳定模式。`115_mount` 需要你已经把 115 网盘目录真正挂载成 Jellyfin Docker 能访问的本地路径；普通“同步文件夹”不算挂载，因为它仍会占用本地空间。
...
`115_mount` 模式下，虚拟影片仍放在 `javplay_db_path`，真实影片放在 115 网盘目录。Jellyfin 媒体库需要同时包含虚拟库目录和 115 挂载目录。
...
```toml
# 存储模式。默认 aria2_cache；想跳过 Aria2 时设为 115_mount
