...
JavPlay 是一个 NoneBot2 插件，用于把 Jellyfin 做成“虚拟影片点播 + 本地缓存”的流程：

1. 从 JavDB 爬取影片条目。
2. 在 Jellyfin 媒体库目录中创建 `.strm` 虚拟影片。
3. Jellyfin 通过 MetaTube 刮削元数据。
4. 用户播放虚拟影片时，插件显示等待视频并开始后台下载。
5. 插件把磁力链提交到 115 离线。
6. 插件从 115 获取下载直链并推送到 Aria2。
7. Aria2 下载真实文件到 Jellyfin 媒体目录。
8. 插件刷新单个 Jellyfin 媒体项，确认真实影片出现后删除虚拟 `.strm`。
...
| 使用者 | 配置项 | 示例 | 说明 |
| --- | --- | --- | --- |
| NoneBot 插件所在机器 | `javplay_db_path` | `/PATH_ON_BOT_HOST/JAV` | 插件创建 `.strm`、检查下载文件、清理缓存 |
...
| Aria2 容器 | `javplay_aria2_dir` | `/PATH_IN_ARIA2_CONTAINER/JAV` | Aria2 保存下载文件的路径 |
| 115 网盘 | `javplay_115_savepath` | `/PATH_IN_115_CLOUD/JAV` | 115 离线任务保存路径 |
...
如果这三个路径没有映射到同一个目录，常见现象是：Aria2 显示下载完成，但 Jellyfin 看不到影片；或插件一直等待真实文件出现。
...
Jellyfin 播放这个地址时，会看到插件目录中的 `wait.mp4`。插件会结合 Jellyfin 当前活跃播放会话确认用户真的在播放该番号，确认后才会加入下载队列，避免 Jellyfin 客户端预加载旧视频导致误触发。

## 文件结构

```text
nonebot_plugin_javplay_public/
├── __init__.py              # 插件入口、路由、命令、调度、下载编排
├── config.py                # NoneBot .env 配置模型
├── scraper.py               # JavDB 爬取与磁力链搜索
├── library_builder.py       # 虚拟库创建、全量扫描、清理恢复
├── downloader_115.py        # 115 登录、离线、直链获取
...
| `nonebot-plugin-apscheduler` | 每日爬取和本地清理定时任务 |
| `beautifulsoup4` | 解析 JavDB 页面 |
| `httpx` | 异步 HTTP 请求 |
| `requests` | 115 ProAPI 和回调脚本 |
| `p115client` | 115 登录、离线、网盘文件查询 |
...
| `javplay_115_savepath` | 是 | 115 离线保存目录 |
| `javplay_proxy_http` | 否 | 直接请求 JavDB 时使用的 HTTP 代理 |
...
| `javplay_aria2_dir` | 是 | Aria2 容器内下载目录 |
...
| `javplay_crawl_pages_daily` | 否 | 每日自动爬取最新几页 |
...
JavDB 可能有 Cloudflare 验证，插件通过 FlareSolverr 获取页面。
...
如果 FlareSolverr 所在机器访问 JavDB 也需要代理：
...
插件通过 Aria2 JSON-RPC 添加 115 下载直链。
...
4. 获取大文件直链时，可能额外保存 `115_download_cookie.txt`。

这些文件都被 `.gitignore` 忽略，不要上传。

如果 Cookie 过期，删除插件目录中的 `115_cookie.txt` 和 `115_download_cookie.txt`，下次触发下载会重新扫码。
...
测试爬取最新少量页面。可用于确认 JavDB、FlareSolverr、路径和 Jellyfin 刷新是否正常。

```text
完全扫描jav
```

持续扫描 JavDB 历史页面。扫描进度会写入插件目录下的 `page.json`，Bot 重启后继续上次进度。

## 定时任务

安装 `nonebot-plugin-apscheduler` 后，插件会注册：

- 每天爬取最新 `javplay_crawl_pages_daily` 页。
...
- 初始状态：只有 `.strm` 虚拟影片。
- 用户点播：下载真实视频到同一番号目录。
- 下载完成：刷新 Jellyfin，确认真实媒体项出现后删除 `.strm`。
- 清理过期缓存：删除本地真实视频，但不删除 115 网盘内容。
- 清理后：重新创建 `.strm`，保留 Jellyfin 点播入口。

清理任务不会清空整个媒体库目录，也不会删除 115 离线文件。
...
- FlareSolverr 是否能打开 JavDB。
- `javplay_flaresolverr_url` 是否能被 NoneBot 访问。
- FlareSolverr 所在机器是否需要代理。
- 日志里的页面标题是否是 404、超时、验证页。

### Jellyfin 能播放等待视频，但没有开始下载

插件会确认 Jellyfin 当前活跃播放会话中的番号与 `wait.mp4?video_id=番号` 一致。没有匹配会话时不会下载。
...
### 下载完成但 Jellyfin 看不到真实影片

几乎都是路径映射问题。确认：

- Aria2 任务里的下载路径是 `javplay_aria2_dir/番号/番号.mp4`。
...
正常。插件会把 115 重复离线任务视为可继续状态，然后搜索 115 已有文件。
